...
class Attention(nn.Module):
    ...
    def forward(self, x):
        B, N, C = x.shape
        # print('x before qkv', x)
        # print(self.qkv)
        # print('x after linear qkv', self.qkv(x))
        ...
        # print('attn after dropout', attn)
        if torch.any(torch.isnan(attn)):
            exit(0)

        x = (attn @ v).transpose(1, 2).reshape(B, N, C)
        # print('x after attn @ v', x)
        x = self.proj(x)
        # print('x after proj', x)
        x = self.proj_drop(x)
        # print('x after proj_drop', x)
        return x
...
class Block(nn.Module):
    ...
    def forward(self, x):
        # print('norm1 x', self.norm1(x))
        x = x + self.drop_path(self.attn(self.norm1(x)))
        # print('x in block', x)
        # print('norm2 x', self.norm2(x))
...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts):
        ...
        # print('group_input_tokens', group_input_tokens)
        # print('center', center)
        # prepare cls
...
#     config = AutoConfig.from_pretrained('q-future/one-align')
