...
def load_pretrained_model(model_path, model_base, model_name, point_backbone_ckpt=None, load_8bit=False, load_4bit=False, device_map="auto", device="cuda"):
    ...
    if True: # 'align' in model_name.lower(): # pit-qmm
        # print('Qalign ========')
        # Load LLaVA model
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            model.get_model().load_point_backbone_checkpoint(point_backbone_ckpt)
            model.initialize_tokenizer_point_backbone_config(tokenizer=tokenizer, device=device, fix_llm=True)
